...
class IntensityBuilder:
    ...
    def _create_intensity(self, recipe: dict):
        ...
        if intensity_class in basic_intensity_builders:
            return basic_intensity_builders[intensity_class](recipe,
                                                             self)
...
class _CoherentIntensity():
    ...
    def __call__(self, dataset):
        return tf.pow(
            tf.cast(
                tf.abs(
                    tf.add_n([amp(dataset) for amp in self.amps])
                ),
                dtype=tf.float64),
...
class _HelicityDecay():
    ...
    def _with_dynamics(self, dataset):
        return wigner_capital_d(dataset[self._phi_name],
                                dataset[self._theta_name], 0.0,
                                2 * self._j, 2 * self._m, 2 * self._mprime) \
            * self._dynamics_function(dataset[self._inv_mass_name],
