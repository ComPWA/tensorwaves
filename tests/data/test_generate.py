...
def test_generate_data(data_sample: MomentumPool):
    sub_sample = {
        i: four_momenta[:5, :] for i, four_momenta in data_sample.items()
    }
    print("Expected shape, get by running pytest with the -s flag")
    pprint(
        {
    ...
    assert set(sub_sample) == set(expected)
    for i in sub_sample:
        assert pytest.approx(sub_sample[i]) == expected[i]
...
def test_generate_phsp(
    ...
    assert len(sample) == len(final_state_names)
    for four_momenta in sample.values():
        assert len(four_momenta) == sample_size
